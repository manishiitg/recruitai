...
def process(data):
    ...
    for rowIdx in tqdm(range(len(data))):
        row = data[rowIdx]
    ...
    for pageno, pagedata in enumerate(nerparsed):
        ...
        for lineno, line in enumerate(pagedata):
                ...
                for ent in line["entity"]["entities"]:
                    ...
                    if enttype in extractEntity[pageno][lineno]:
                        if start_pos - prevPos <= 3:
                            ...
                            # new enitty
                            prevPos = end_pos
                            extractEntity[pageno][lineno][enttype].append(text)
                    ...
                    logger.info(line["entity"]["entities"])
                    ...
                    if "ORG" in extractEntity[pageno][lineno]:
                        if "Designation" in extractEntity[pageno][lineno] and "EducationDegree" in extractEntity[pageno][lineno]:
                            logger.info("this has both designation and degree")
                            logger.info(
            ...
            logger.info(json.dumps(extractEntity, indent=1, sort_keys=False))
...
def updateSingleEntity(singleEntity, entityLine, finalEntity):
    foundEntity = False
