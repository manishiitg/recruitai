...
def queue_process():
    ...
    summary_instance_name = 'torchsummary'

    queues = get_queues()
    if "summary" in queues:
        if int(queues["summary"]['in_process']) > 3400:
            

            instance_status = check_compute_running(summary_instance_name)
            ...
            if len(instance_status) > 0:
                ...
                start_compute_preementable(summary_instance_name)
        else:
            LOGGER.critical("summary has less than 100 no need to start gpu %s", queues["summary"])
        ...
        LOGGER.critical("summary not found in queues")
...
def check_compute_running(instance_name):
    ...
    LOGGER.critical("number of connections on rabbit mq %s", len(connections))
...
def start_compute_preementable(instance_name):
    zones = get_zone_list()
    max_attemps = 10
    for idx, zone in enumerate(zones):
        LOGGER.critical("trying to start gpu for instance name %s for zone regision %s", instance_name+ str(idx), zone["name"])
        started = start_compute(instance_name + str(idx), zone["name"])
        if started:
...
def start_compute(instance_name, zone):
