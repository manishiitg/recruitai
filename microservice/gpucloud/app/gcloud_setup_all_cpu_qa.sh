...
sudo docker-compose -f docker-compose-cpu-qa.yml pull --quiet
sudo docker-compose -f docker-compose-cpu-qa.yml up -d --scale=qamq=4

for i in `seq 1 1000`;
do
  sleep 30m
  echo "restarting docker $i"
  sudo docker-compose -f docker-compose-cpu-qa.yml restart
